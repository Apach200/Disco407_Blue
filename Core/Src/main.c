	  ...
	  HAL_Delay(100);
		  ...
		  while (1)
		  {
			  ...
			  canopen_app_process();
			  			...
			  			if(tmp32u_0 != OD_PERSIST_COMM.x6001_disco_Blue_VAR32_6001R)
			  			{
			  			tmp32u_0 = OD_PERSIST_COMM.x6001_disco_Blue_VAR32_6001R;

			  			TerminalInterface.gState = HAL_UART_STATE_READY;
			  			HAL_UART_Transmit_DMA( &TerminalInterface, (uint8_t*)(&tmp32u_0), 4);
			  			}

			  			if(tmp32u_1 != OD_PERSIST_COMM.x6002_disco_Blue_VAR32_6002R)
			  			{
			  			tmp32u_1 = OD_PERSIST_COMM.x6002_disco_Blue_VAR32_6002R;
			  			TerminalInterface.gState = HAL_UART_STATE_READY;
